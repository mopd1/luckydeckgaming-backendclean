...
class PokerGameEngine {
    ...
    // Count players who can still act (not folded, have chips)
    countActivePlayers() {
        let count = 0;
    ...
    // Initialize dealer position to first available player
    initializeDealerPosition() {
        for (let i = 0; i < 5; i++) {
            if (this.players[i] && !this.players[i].folded) {
                this.dealerPosition = i;
                console.log(`🔘 Initialized dealer position to seat ${i}`);
                return;
            }
        }
        console.log('🚨 ERROR: No active players found to set dealer position');
        this.dealerPosition = 0; // Fallback
    }
    ...
    async startNewHand() {
        ...
        // Ensure dealer position is valid, especially for first hand
        if (!this.players[this.dealerPosition] || this.players[this.dealerPosition].folded) {
            console.log(`🔘 Current dealer position ${this.dealerPosition} is invalid, initializing...`);
    ...
    setFirstAction() {
        ...
        if (activePlayers.length === 2) {
            ...
            if (position === -1) {
                console.log('🚨 ERROR: Could not find small blind in heads-up');
                this.actionOn = -1;
                return;
            }
            position = this.findNextActivePlayer(position);
            if (position === -1) {
                console.log('🚨 ERROR: Could not find big blind in heads-up');
                this.actionOn = -1;
                return;
            }
            this.actionOn = position;
            console.log(`🎯 Heads-up: Action starts with big blind at seat ${position}`);
            return;
        }
        ...
        if (position === -1) {
            console.log('🚨 ERROR: Could not find small blind position');
            this.actionOn = -1;
            return;
        }
        position = this.findNextActivePlayer(position); // Big blind
        if (position === -1) {
            console.log('🚨 ERROR: Could not find big blind position');
            this.actionOn = -1;
            return;
        }
        position = this.findNextActivePlayer(position); // UTG
        if (position === -1) {
            console.log('🚨 ERROR: Could not find UTG position');
            this.actionOn = -1;
            return;
        }
        
        this.actionOn = position;
        console.log(`🎯 Multi-way: Action starts with UTG at seat ${position}`);
    }
    ...
    findNextActivePlayer(startPosition) {
        ...
        console.log(`🔍 findNextActivePlayer: Starting from seat ${startPosition}, checking from seat ${position}`);
        
        while (attempts < 5) {
            const player = this.players[position];
            console.log(`   Checking seat ${position}: ${player ? `${player.username} (folded: ${player.folded})` : 'EMPTY'}`);
            
            if (player && !player.folded) {
                console.log(`   ✅ Found active player at seat ${position}: ${player.username}`);
                return position;
            }
        ...
        console.log(`   ❌ No active player found after checking all seats from ${startPosition}`);
        return -1; // No active player found
    }

    // More robust method to find next player who needs to act in betting
    findNextPlayerToAct(startPosition, currentBet) {
        let position = (startPosition + 1) % 5;
        let attempts = 0;
        
        console.log(`🎯 findNextPlayerToAct: Starting from seat ${startPosition}, currentBet=${currentBet}`);
        
        while (attempts < 5) {
            const player = this.players[position];
            
            if (player && !player.folded && player.chips > 0) {
                // Player needs to act if they haven't matched the current bet or haven't had their turn
                const needsToAct = player.bet < currentBet || !player.had_turn_this_round;
                console.log(`   Seat ${position} (${player.username}): bet=${player.bet}, had_turn=${player.had_turn_this_round}, needsToAct=${needsToAct}`);
                
                if (needsToAct) {
                    console.log(`   ✅ Found player who needs to act at seat ${position}: ${player.username}`);
                    return position;
                }
            }
        ...
        console.log(`   ❌ No player found who needs to act from ${startPosition}`);
        return -1;
    }
    ...
    async moveToNextPlayer() {
        ...
        // Find next player to act using robust helper method
        ...
        // Use the robust findNextPlayerToAct method
        nextSeat = this.findNextPlayerToAct(startSeat, highestBet);
        ...
        if (nextSeat === -1) {
            ...
            // Emergency: try to find ANY active player using findNextActivePlayer
            console.log(`   🔧 EMERGENCY: Searching for ANY active player...`);
            nextSeat = this.findNextActivePlayer(startSeat);
            
            if (nextSeat !== -1) {
                console.log(`   🔧 EMERGENCY: Found active player at seat ${nextSeat}`);
            }
    ...
    setPostFlopAction() {
        ...
        const nextPosition = this.findNextActivePlayer(this.dealerPosition);
        if (nextPosition !== -1) {
            this.actionOn = nextPosition;
            console.log(`Post-flop action starts with seat ${nextPosition}`);
        } else {
            console.log('🚨 ERROR: Could not find active player for post-flop action');
            this.actionOn = -1;
        }
    }
